$(document).ready(function() {
    layui.use(['form', 'table','layer'], function() {
        ...
        table.render({
            ...
            height: 500,
            url: '/roomlock/manager/list',
            cols: [[
                
                {field: 'roomId',  title: '房间id'},
                {field: 'lockDate',  title: '锁定日期'},
                {field: 'lockSlot',  title: '锁定槽'},
        ...
        $("#add_roomlock_btn").click(function() {
           window.location.href = '/roomlock/manager/add';
        ...
        table.on('tool(listDataTable01Filter)', function(obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                window.location.href = "/roomlock/manager/detail/" + data.id;
            } else if (obj.event === 'edit') {
                window.location.href = "/roomlock/manager/update/" + data.id;
            } else if (obj.event === 'delete') {
                layer.confirm("确定删除？", {icon: 3}, function(index) {
                    $.ajax({
                        url: "/roomlock/manager/delete/" + data.id,
                        type: "DELETE",
                        dataType: "json",
                        success: function(data) {
                            if (data.success) {
